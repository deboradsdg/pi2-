<?php
// Arquivo: Principal.php (Deve estar na raiz do seu projeto)
...
<body>
    <script>
        // Define 'clienteLogado' como true apenas se houver sessão E o tipo for 'cliente'
        const clienteLogado = <?php echo $is_logged_in && $user_type === 'cliente' ? 'true' : 'false'; ?>;
        // Define 'userId' como o ID do usuário ou 'null'
        const userId = <?php echo $is_logged_in ? ($_SESSION['user_id'] ?? 'null') : 'null'; ?>;
    </script>
    <header>
        <div id="LT">
            <img src="../MIDIAS/IMAGENS/PRINCIPAL.png" alt="Logo The Pizza One">
        </div>
        <a href="addadmin.php">Adicionar Admin</a>
        <div id="status-login">
            <?php if ($is_logged_in): ?>
                <span>Olá, <?php echo $user_name; ?>!</span>
                <?php if ($is_admin): ?>
                    <a href="../admin/painel_admin.php" class="btn-status">Painel Admin</a>
                <?php endif; ?>
                <a href="../PHP/logout.php" class="btn-status">Sair</a>
            <?php else: ?>
